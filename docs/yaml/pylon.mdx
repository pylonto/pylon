---
title: "pylon.yaml"
description: "Complete reference for the per-pylon configuration file. Defines triggers, workspaces, agent prompts with template variables, notification channels, and volume mounts for each Pylon pipeline."
---
...
<ParamField path="agent.type" type="string">
  Agent runtime for this pylon. Accepted values: `claude`, `opencode`, `pi`. Pi requires explicit configuration and the isolated [subscription worker](/running/pi-worker), not the ordinary daemon.
</ParamField>

<ParamField path="agent.pi" type="PiConfig">
  Pi-only configuration: immutable `image`, role-local `auth_dir`, 1--32 `allowed_paths`, and explicit `limits.daily_jobs`, `limits.daily_tokens`, `limits.job_tokens`, and `limits.job_seconds`. Optional `thinking` accepts `medium` or `max`; absent defaults to `max`. This is a per-job worker setting, not a global or brief-controlled override. Optional `debug_dir` enables [private bounded inspection](/running/pi-worker#inspect-a-private-repair-capture) in a precreated canonical `0700` directory; absent or empty disables it. See the [worker configuration and bounds](/running/pi-worker#configure-signed-admission). Pi rejects the generic prompt, API key, environment, mount, and timeout fields below.
</ParamField>
...
<ParamField path="agent.volumes" type="string[]">
...
<Step title="Trigger the pylon at least once">
...
See [Nexus message template builder](/concepts/nexus#message-template-builder) for the full keybinding reference.
...
The mode is optional and defaults to `ro` (read-only). Set it to `rw` if the tool needs to write to the mounted directory.
...
There are two common ways to pass credentials into agent containers:

**Environment variables** -- for service accounts with static tokens. Store tokens in `~/.pylon/.env` and reference them in the `env` block:
