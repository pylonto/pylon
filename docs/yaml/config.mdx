...
Pylon supports two levels of secrets files:

**Global** -- `~/.pylon/.env` for secrets shared across all pylons:

```bash
ANTHROPIC_API_KEY=sk-ant-...
```

**Per-pylon** -- `~/.pylon/pylons/<name>/.env` for secrets scoped to a single pylon:
...
Per-pylon secrets take priority over global ones when the same variable name appears in both files. Reference these values in `config.yaml` or `pylon.yaml` using `${VAR_NAME}` syntax. Pylon expands them at startup, so the config files never contain raw secrets.

<Tip>
Use per-pylon `.env` files to keep secrets organized. Each pylon can use clean, generic variable names like `TELEGRAM_BOT_TOKEN` without worrying about collisions with other pylons.
</Tip>
