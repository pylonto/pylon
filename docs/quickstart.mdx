...
<Steps>
  ...
  <Step title="Start the daemon">
    ...
    <Warning>
      `pylon start` runs in the foreground. To run it in the background, use a process manager like systemd. See [Daemon](/running/daemon) for a systemd unit example.
    </Warning>
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="YAML reference" icon="gear" href="/yaml/pylon">
