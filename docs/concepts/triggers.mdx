...
Pylon accepts a **bare lowercase hex digest**, or GitHub's `sha256=<hex>` format when the configured header is `X-Hub-Signature-256`. A configured secret without a signature header, or a secret that expands to an empty value, fails closed.

For [durable deliveries](/running/durable-deliveries), include `Idempotency-Key` and sign `key + "\n" + body` instead. The receiver persists keyed work before acknowledging it and retains it when agents are busy. Bodies are limited to 64 KiB.
...
| GitHub | `X-Hub-Signature-256` | `sha256=<hex>` | Yes |
| Custom sender | any | bare hex | Yes, when you control the sender |

For Slack, validate its native timestamped signature in a trusted intermediary. Keep Pylon private behind it; do not disable verification on a publicly reachable receiver.
