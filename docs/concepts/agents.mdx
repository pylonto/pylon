...
Pylon supports **Claude Code** and **OpenCode** in the ordinary daemon, plus **Pi** in a separate [subscription repair role](/running/pi-worker). You configure an agent under the `agent` key in `pylon.yaml`. If you omit the `agent` block, Pylon falls back to the global defaults in `~/.pylon/config.yaml`; Pi requires explicit per-pylon configuration.

The fields and prompt examples below describe Claude Code and OpenCode. Pi accepts only signed maintenance briefs and rejects their prompt, environment, mount, and timeout overrides.
